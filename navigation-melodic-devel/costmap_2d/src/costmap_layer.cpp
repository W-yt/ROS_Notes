...
namespace costmap_2d{
  ...
  //CostmapLayer类继承自Layer类和Costmap2D类，它是地图插件（静态层和障碍层）的基类
  //CostmapLayer类的类方法主要用于处理bound和用几种不同的策略合并子地图和主地图
  ...
  //该函数用当前子地图数据（不包括未知cell）更新主地图对应区域
  ...
  //该函数用当前子地图数据（包括未知cell）覆盖主地图对应区域
  ...
  //该函数用当前子地图数据（不包括未知cell）覆盖主地图对应区域
  ...
  //该函数用当前子地图数据（不包括未知cell）更新主地图对应区域
  //若主地图该cell为未知，用子地图数据覆盖；否则，在主地图原数据基础上+子地图数据
