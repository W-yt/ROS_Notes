...
核心函数：
\begin{itemize}
    \item [-] 构造函数 MoveBase::MoveBase 
    \item [-] 控制主体(回调函数) MoveBase::executeCb
    \item [-] 全局规划线程 MoveBase::planThread
    \item [-] 全局规划函数 MoveBase::makePlan
    \item [-] 局部规划函数 MoveBase::executeCycle
    \item [-] 析构函数 MoveBase::~MoveBase
\end{itemize}
    
其他函数：
\begin{itemize}
    \item [-] 动态加载配置参数 MoveBase::reconfigureCB
    \item [-] 仿真时发布目标函数 MoveBase::goalCB
    \item [-] 可以自动搜索终点附近可达到点的路径规划服务 MoveBase::planService
    \item [-] 发布零速度 MoveBase::publishZeroVelocity
    \item [-] 检查四元数合法性函数 MoveBase::isQuaternionValid
    \item [-] 将目标点转换到全局坐标系下 MoveBase::goalToGlobalFrame
    \item [-] 唤醒路径规划线程 MoveBase::wakePlanner
    \item [-] 获取两点之间的水平路径 MoveBase::distance
    \item [-] 加载恢复行为插件 MoveBase::loadRecoveryBehaviors
    \item [-] 加载默认恢复行为插件 MoveBase::loadDefaultRecoveryBehaviors
    \item [-] 路劲规划系统复位 MoveBase::resetState
    \item [-] 获取在指定代价地图下的位姿 MoveBase::getRobotPose
\end{itemize}
